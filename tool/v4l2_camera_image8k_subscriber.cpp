...
private:
  using Topic = shm_msgs::msg::Image8k;

public:
  explicit Listener(const rclcpp::NodeOptions &options)
      : Node("shm_image8k_subscriber", options) {
    ...
    auto callback = [this](const Topic::SharedPtr msg) -> void {
      ...
      {
        RCLCPP_INFO(get_logger(), "get-image8k-timestamp_offset-time: %.3f", timestamp_offset_ms);
      }
      m_last_image_ts = msg->header.stamp;
      RCLCPP_INFO(get_logger(), "get-image8k-transport-time: %.3f", time_offset_ms);
    ...
    m_subscription = create_subscription<Topic>("shm_image_8k", qos, callback);
  }
