...
private:
  using Topic = shm_msgs::msg::Image8m;

public:
  explicit Listener(const rclcpp::NodeOptions &options)
      : Node("shm_image8m_subscriber", options) {
    ...
    auto callback = [this](const Topic::SharedPtr msg) -> void {
      ...
      RCLCPP_INFO(get_logger(), "get-image8m-transport-time: %.3f", time_offset_ms);
      if(m_last_image_ts.nanoseconds() > 0.0)
      {
        RCLCPP_INFO(get_logger(), "get-image8m-timestamp_offset-time: %.3f", timestamp_offset_ms);
      }
    ...
    m_subscription = create_subscription<Topic>("shm_image_8m", qos, callback);
  }
